...
{
    /// <summary>
    /// Consumer service for processing single guest invitation messages from RabbitMQ.
    /// Listens to the message.sending.single queue and dispatches messages to WhatsApp providers.
    /// </summary>
    public class MessageSendingSingleQueueConsumerService : IMessageSendingSingleQueueConsumerService
    {
        private readonly IConnectionFactory _connectionFactory;
        private readonly IServiceScopeFactory _serviceScopeFactory;
        private readonly string QueueName = "MessageSendingSingle";

        public MessageSendingSingleQueueConsumerService(
        ...
        public void ConsumeMessage()
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                try
                {
                    ...
                    if (request == null)
                    {
                        Log.Warning("Received null MessageSendingRequest from single queue");
                    ...
                    using (var scope = _serviceScopeFactory.CreateScope())
                    {
                        ...
                        using (var db = dbFactory.CreateDbContext())
                        {
                            ...
                            if (eventData == null)
                            {
                                Log.Warning($"Event {request.EventId} not found for single MessageSendingRequest");
                                channel.BasicAck(deliveryTag: ea.DeliveryTag, multiple: false);
                                return;
                            }
                            ...
                            Log.Information($"Processed single {request.MessageType} message for Event {request.EventId}, Guests: {string.Join(",", request.GuestIds)}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    Log.Error(ex, "Error processing single MessageSendingRequest from RabbitMQ");
                }
